...
const viewport = BUI.Component.create(() => {
    return BUI.html `
    <bim-viewport>
...
const viewportGrid = viewport.querySelector("bim-grid[floating]");
...
window.addEventListener("message", async (event) => {
    if (event.data?.fileExtension === "ifc") {
        const model = await ifcLoader.load(event.data?.buffer);
        world.scene.three.add(model);
    }
});
